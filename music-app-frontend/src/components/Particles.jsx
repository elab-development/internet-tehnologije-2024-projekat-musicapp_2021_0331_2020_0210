...
export default function Particles({
  ...
  useEffect(() => {
    // If there was an error, don't try to initialize again
    if (error) return;

    const mount = containerRef.current;
    if (!mount) return;

    // Import OGL dynamically to handle possible initialization failures
    import("ogl").then(({ Renderer, Camera, Transform, Geometry, Program, Mesh }) => {
      try {
        ...
        return () => {
          ...
          if (mount && gl && gl.canvas && mount.contains(gl.canvas)) {
      ...
      console.error("Failed to load OGL library:", err);
      setError(true);
    });
  }, [
